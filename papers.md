...
1. {{paper.author}}. {% if paper.link %}[{{paper.title}}]({{paper.link}}){% else %}{{paper.title}}{% endif %}{% assign last_char = paper.title | slice: -1 %}{% unless last_char == '?' %}.{% endunless %} _{{paper.conference}}_.{% if paper.bibtex %}
